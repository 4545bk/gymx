...
export default function CheckinContent() {
  ...
  const [apiBase, setApiBase] = useState(process.env.NEXT_PUBLIC_API_URL || '/api/v1');

  useEffect(() => {
    setMounted(true);
    // Smart API URL: direct backend for local/LAN dev, Next.js proxy for production
    const host = window.location.hostname;
    const isLocal = host === 'localhost' || /^192\.168/.test(host) || /^10\./.test(host);
    setApiBase(
      process.env.NEXT_PUBLIC_API_URL
      || (isLocal ? `http://${host}:5000/api/v1` : '/api/v1')
    );
  }, []);
  ...
  useEffect(() => {
    const goOnline = async () => {
      ...
      if (count > 0) {
        setSyncing(true);
        await syncPendingCheckins(SCANNER_KEY, apiBase);
  ...
  useEffect(() => {
    ...
    const url = `${apiBase}/checkin/stream?token=${token}`;
  ...
  }, [apiBase, isOnline, mounted, fetchToday, fetchOfflineSyncCount]);

  // ─── Fallback sync polling (no Background Sync) ──────────
  useEffect(() => {
    registerSyncFallback(SCANNER_KEY, apiBase, (remaining) => {
  ...
  const handleManualSync = useCallback(async () => {
    setSyncing(true);
    await manualSync(SCANNER_KEY, apiBase);
    setPendingSync(await getPendingCount());
    await fetchToday();
    await fetchOfflineSyncCount();
    setSyncing(false);
  }, [SCANNER_KEY, apiBase, fetchToday, fetchOfflineSyncCount]);
  ...
  }, [apiBase, SCANNER_KEY, scanning, playSound, isOnline]);
  ...
  // only render the full UI after client mount.
    ...
    <div style={{
      ...
      <div style={{
        ...
        <div style={{ display: 'flex', alignItems: 'center', gap: 12 }} suppressHydrationWarning>
          <Link href="/dashboard" style={{ color: 'rgba(255,255,255,0.3)', textDecoration: 'none', display: 'flex', alignItems: 'center', gap: 4, fontSize: 13 }}><ArrowLeft size={14} /><span>{t('checkin.backToDashboard')}</span></Link>
          <Link href="/checkin/mobile" style={{
            ...
            fontWeight: 500
          }}><Smartphone size={13} /><span>{t('nav.mobileScanner')}</span></Link>
          <span style={{ fontSize: 16, fontWeight: 600, color: '#E8F5EE', marginLeft: 8 }}>GymX</span>
        </div>
        <div style={{ display: 'flex', alignItems: 'center', gap: 16 }}>
          {/* Connection status */}
          <div style={{ display: 'flex', alignItems: 'center', gap: 6, fontSize: 12 }} suppressHydrationWarning>
            <div style={{
          ...
          {offlineSyncCount > 0 && (
            ...
            }}><CloudOff size={10} style={{ marginRight: 4, verticalAlign: 'middle' }} /><span>{offlineSyncCount} synced offline</span></span>
          )}
          {/* Mute toggle */}
          ...
          {pendingSync > 0 && (
            ...
            >
              ...
              <span>⚠ {pendingSync} queued — {syncing ? 'Syncing...' : 'Sync now'}</span>
            </button>
          )}
        </div>
      </div>
      ...
      {!isOnline && (
        ...
        }}><WifiOff size={14} /><span>No internet connection — check-ins are being saved locally and will sync automatically.</span></div>
      )}
      ...
      {isOnline && !connected && sseStatus !== 'disconnected' && (
        ...
        }}><RefreshCw size={12} style={{ animation: 'spin 2s linear infinite' }} /><span>{sseStatus === 'polling' ? 'Live feed unavailable — polling for updates every 10s' : 'Live feed paused — reconnecting...'}</span></div>
      )}
        ...
        <div style={{
          ...
          {scanning ? (
            ...
            <div style={{ textAlign: 'center' }}>
              ...
              }}><CheckCircle size={14} /><span>{totalToday} {t('checkin.checkinsToday')}</span></div>
            </div>
          )}
        </div>
